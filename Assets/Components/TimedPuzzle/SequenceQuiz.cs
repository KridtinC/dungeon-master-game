  ...
  void Update(){
    if(time.timeRun){
      ...
      if(time.timeRun && !puzzle.completed){
        if (dist <= minDist){
          GUI.TextArea(new Rect(Screen.width * 0.125f, Screen.width * 0.125f, 200, 50), "press 'R' nearby number's pillar by following order "
          +string.Join(",", seq0.ToArray())+"\n"+string.Join(",", seq1.ToArray()));
          }
        else if (dist1 <= minDist){
          GUI.TextArea(new Rect(Screen.width * 0.125f, Screen.width * 0.125f, 20, 20), "1");
          }
        else if (dist2 <= minDist){
          GUI.TextArea(new Rect(Screen.width * 0.125f, Screen.width * 0.125f, 20, 20), "2");
          }
        else if (dist3 <= minDist){
          GUI.TextArea(new Rect(Screen.width * 0.125f, Screen.width * 0.125f, 20, 20), "3");
          }
        else if (dist4 <= minDist){
          GUI.TextArea(new Rect(Screen.width * 0.125f, Screen.width * 0.125f, 20, 20), "4");
          }
        }
  }
}
